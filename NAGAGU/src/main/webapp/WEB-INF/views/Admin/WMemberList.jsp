...
<script>
		...
		$.ajax({
			...
			success:function(data) {
				...
				$.each(data, function(index, item) {
					...
					output += '<tr>';
					output += '<td>' + startnum + '</td>'; // undefined
					output += '<td><a href="./detailWMember.ad" class="detail_data" '
					output += 'WORKSHOP_NUM=' + item.workshop_NUM + '>' + item.workshop_EMAIL + '</a></td>';
	...
	$(document).on('click', '.detail_data', function(event) {
		...
		jQuery.ajax({
			...
			success : function (retVal) {
				if (retVal.res == "OK") {
					pop.location.href="WmemberInfo.ad?WORKSHOP_NUM=" + retVal.WMemberVO.workshop_NUM;	
		 		}
			},
			error: function(request,status,error) {
