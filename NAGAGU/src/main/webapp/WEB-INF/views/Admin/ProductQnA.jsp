...
<div id="page-content-wrapper" style="padding-top: 5%;">
	<div class="container-fluid">
		...
		<div class="d-flex justify-content-center">
			<nav aria-label="Page navigation example" class="paginated" id="productQnA-page"></nav>
		</div>
	</div>
</div>
<!-- /본문 -->
...
<script>
		...
		$.ajax({
			...
			success:function(data) {
				...
				$.each(data, function(index, item) {
					...
					if(item.QNA_CONTENT.length >= 60) {
						content = item.QNA_CONTENT.substr(0,60)+"...";
	...
	$(document).on('click', '.del_data', function(event) {
		...
		jQuery.ajax({
			...
			type : 'GET',
			data : {'QNA_NUM' : $(this).attr("qna_NUM")},
			contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
			dataType : 'json',
...
// 	});
	...
	function page() { 	
		...
		$('nav.paginated').each(function() {
			var pagesu = 10;  //페이지 번호 갯수
	  		...
	  		var $table = $('#product-qna');    
	  		var $user = $('#productQnA-page');
	...
	$(document).ready(function() {
		$("#keyword").keyup(function() {
			var k = $(this).val();
			$("#product-qna > tbody > tr").hide();
			var temp = $("#product-qna > tbody > tr > td:nth-child(10n+2):contains('" + k + "')");
