...
<%
	ArrayList<NoteVO> boxedList = (ArrayList<NoteVO>)request.getAttribute("boxedList");
	int countBoxed = ((Integer)request.getAttribute("countBoxed")).intValue();
	int nowpage = ((Integer)request.getAttribute("page")).intValue();
	int maxpage = ((Integer)request.getAttribute("maxpage")).intValue();
	int startpage = ((Integer)request.getAttribute("startpage")).intValue();
	int endpage = ((Integer)request.getAttribute("endpage")).intValue();
	String redirection = "boxedList.nt";
...
<body>
            ...
            <div class="pop_content">
                <div class="content_top">
                    <div class="top_list_opt">
                        <button type="button" class="btn_note" id="btn_unboxing">
                            선택 보관해제
                        </button>
                    ...
                    <div class="top_list_info">
                        <strong class="note_total">
                            전체
                            <span class="note_num" id="count_total"> <%=countBoxed %>개 </span>
                        </strong>
                    </div>
                </div>
                        	...
                        	%>
                        		<tr>
                        			<td class="no_result" colspan=5>
                        				<div class="no_note_area">보관한 쪽지가 없습니다</div>
                        			</td>
                        		<tr>
                        	<%
                        	} else {
                        		for (int i=0; i<boxedList.size(); i++) {
                            		NoteVO nl = (NoteVO)boxedList.get(i);
    ...
    <script>
	    ...
	    $("#btn_unboxing").click(function(){
	    	if ($("input[name='chk']:checked").length==0) {
	    		return false;
	    	}
	    	$("#selectForm").attr("action", "noteUnboxing.nt");
	    	$("#selectForm").submit();
	    });
