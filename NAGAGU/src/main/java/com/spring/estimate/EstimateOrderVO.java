package com.spring.estimate;

import java.util.Date;

public class EstimateOrderVO {
	private int ES_ORDER_NUM;
	private int ES_ORDER_ESTIMATE;
	private int ES_ORDER_OFFER;
	private String ES_ORDER_TITLE;
	private String ES_ORDER_BUYER;
	private String ES_ORDER_WORKSHOP;
	private String ES_ORDER_PIC;
	private int ES_ORDER_PRICE;
	private int ES_ORDER_SHIP;
	private Date ES_ORDER_DATE;
	private String ES_ORDER_ZIP;
	private String ES_ORDER_ADDRESS;
	private String ES_ORDER_RECEIVER;
	private int ES_ORDER_TRANSFER;
	private int ES_ORDER_STATE;
	
	public int getES_ORDER_NUM() {
		return ES_ORDER_NUM;
	}
	public void setES_ORDER_NUM(int eS_ORDER_NUM) {
		ES_ORDER_NUM = eS_ORDER_NUM;
	}
	public int getES_ORDER_ESTIMATE() {
		return ES_ORDER_ESTIMATE;
	}
	public void setES_ORDER_ESTIMATE(int eS_ORDER_ESTIMATE) {
		ES_ORDER_ESTIMATE = eS_ORDER_ESTIMATE;
	}
	public int getES_ORDER_OFFER() {
		return ES_ORDER_OFFER;
	}
	public void setES_ORDER_OFFER(int eS_ORDER_OFFER) {
		ES_ORDER_OFFER = eS_ORDER_OFFER;
	}
	public String getES_ORDER_TITLE() {
		return ES_ORDER_TITLE;
	}
	public void setES_ORDER_TITLE(String eS_ORDER_TITLE) {
		ES_ORDER_TITLE = eS_ORDER_TITLE;
	}
	public String getES_ORDER_BUYER() {
		return ES_ORDER_BUYER;
	}
	public void setES_ORDER_BUYER(String eS_ORDER_BUYER) {
		ES_ORDER_BUYER = eS_ORDER_BUYER;
	}
	public String getES_ORDER_WORKSHOP() {
		return ES_ORDER_WORKSHOP;
	}
	public void setES_ORDER_WORKSHOP(String eS_ORDER_WORKSHOP) {
		ES_ORDER_WORKSHOP = eS_ORDER_WORKSHOP;
	}
	public String getES_ORDER_PIC() {
		return ES_ORDER_PIC;
	}
	public void setES_ORDER_PIC(String eS_ORDER_PIC) {
		ES_ORDER_PIC = eS_ORDER_PIC;
	}
	public int getES_ORDER_PRICE() {
		return ES_ORDER_PRICE;
	}
	public void setES_ORDER_PRICE(int eS_ORDER_PRICE) {
		ES_ORDER_PRICE = eS_ORDER_PRICE;
	}
	public int getES_ORDER_SHIP() {
		return ES_ORDER_SHIP;
	}
	public void setES_ORDER_SHIP(int eS_ORDER_SHIP) {
		ES_ORDER_SHIP = eS_ORDER_SHIP;
	}
	public Date getES_ORDER_DATE() {
		return ES_ORDER_DATE;
	}
	public void setES_ORDER_DATE(Date eS_ORDER_DATE) {
		ES_ORDER_DATE = eS_ORDER_DATE;
	}
	public String getES_ORDER_ZIP() {
		return ES_ORDER_ZIP;
	}
	public void setES_ORDER_ZIP(String eS_ORDER_ZIP) {
		ES_ORDER_ZIP = eS_ORDER_ZIP;
	}
	public String getES_ORDER_ADDRESS() {
		return ES_ORDER_ADDRESS;
	}
	public void setES_ORDER_ADDRESS(String eS_ORDER_ADDRESS) {
		ES_ORDER_ADDRESS = eS_ORDER_ADDRESS;
	}
	public String getES_ORDER_RECEIVER() {
		return ES_ORDER_RECEIVER;
	}
	public void setES_ORDER_RECEIVER(String eS_ORDER_RECEIVER) {
		ES_ORDER_RECEIVER = eS_ORDER_RECEIVER;
	}
	public int getES_ORDER_TRANSFER() {
		return ES_ORDER_TRANSFER;
	}
	public void setES_ORDER_TRANSFER(int eS_ORDER_TRANSFER) {
		ES_ORDER_TRANSFER = eS_ORDER_TRANSFER;
	}
	public int getES_ORDER_STATE() {
		return ES_ORDER_STATE;
	}
	public void setES_ORDER_STATE(int eS_ORDER_STATE) {
		ES_ORDER_STATE = eS_ORDER_STATE;
	}
}
